...
if(isset($_POST["da"]))
{
   $obj->select_reportpps($_POST['da'],$_POST['db'],$page);
}
else
{
   $obj->select_reportpps("0000-00-00",$Date,$page);
}
if(mysqli_num_rows($resultreportpps) > 0)
{
 $output .= '
 ...
 ';
 while($row = mysqli_fetch_array($resultreportpps))
 {
  $output .= '
   <tr  class="result">
   <td>'.$row["serial"].'</td>
 ...
 mysqli_free_result($resultreportpps);  
 ...
 else
 {
   ...
   $count = mysqli_num_rows($resultreportpps_count);
   mysqli_free_result($resultreportpps_count);  
   mysqli_next_result($conn);
   $a = ceil($count/15);
   if(isset($_POST['page'])){
      if($_POST['page'] > 1){
