...
if(isset($_POST["da"]))
{
   $obj->select_checkstock($_POST['da'],$_POST['db'],$page);
}
else
{
   $obj->select_checkstock("0000-00-00",$Date,$page);
}
if(mysqli_num_rows($resultcheckstock) > 0)
{
 ...
 ';
 while($row = mysqli_fetch_array($resultcheckstock))
 {
  $output .= '
   <tr  class="result">
 ...
 mysqli_free_result($resultcheckstock);  
 ...
 if(isset($_POST["da"]))
 {
   $obj->select_checkstock_count($_POST['da'],$_POST['db']);
 }
 else
 {
    $obj->select_checkstock_count("0000-00-00",$Date);
 }
   $count = mysqli_num_rows($resultcheckstock_count);
   mysqli_free_result($resultcheckstock_count);  
   mysqli_next_result($conn);
   $a = ceil($count/15);
   if(isset($_POST['page'])){
      if($_POST['page'] > 1){
