...
-- Procedures
--
CREATE  PROCEDURE `auther` (IN `s` VARCHAR(50), IN `page` INT(5))  Begin
...
CREATE  PROCEDURE `customer_count` (IN `s` VARCHAR(50))  NO SQL
begin
...
CREATE  PROCEDURE `customer_status` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
select stt_id,stt_name from customer_status where stt_id like s or stt_name like s ORDER by stt_name ASC limit page,15;
end$$

CREATE  PROCEDURE `customer_status_count` (IN `s` VARCHAR(50))  NO SQL
begin
select stt_id,stt_name from customer_status where stt_id like s or stt_name like s ORDER by stt_name ASC;
end$$

CREATE  PROCEDURE `delete_auther` (`autherid` VARCHAR(20))  Begin
Delete from  auther  where auther_id=autherid;
End$$

CREATE  PROCEDURE `delete_brand` (IN `id` INT(11))  begin
Delete from brand where brand_id=id;
end$$

CREATE  PROCEDURE `delete_category` (`id` INT(11))  begin
Delete from category where cate_id=id;
end$$

CREATE  PROCEDURE `delete_customer` (`id` VARCHAR(20))  begin
Delete from customer where cus_id=id;
end$$

CREATE  PROCEDURE `delete_customer_status` (`id` VARCHAR(20))  begin
Delete from customer_status where stt_id=id;
end$$

CREATE  PROCEDURE `delete_employee` (`empid` VARCHAR(20))  Begin
Delete from  employee  where emp_id=empid;
End$$

CREATE  PROCEDURE `delete_form` (`id` VARCHAR(20))  begin
Delete from form where form_id=id;
end$$

CREATE  PROCEDURE `delete_products` (`id` INT(11))  begin
Delete from products where code=id;
end$$

CREATE  PROCEDURE `delete_product_addr` (IN `id` INT(11))  begin
Delete from product_addr where pro_ad=id;
end$$

CREATE  PROCEDURE `delete_rate` (IN `id` VARCHAR(20))  begin
Delete from rate where rate_id=id;
end$$

CREATE  PROCEDURE `delete_spare_part` (IN `spid` INT(11))  begin
Delete from spare_part where id=spid;
end$$

CREATE  PROCEDURE `delete_supplier` (`id` VARCHAR(20))  begin
Delete from supplier where sup_id=id;
end$$

CREATE  PROCEDURE `delete_unit` (`id` INT(11))  begin
delete from unit where unit_id=id;
end$$

CREATE  PROCEDURE `distribute` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `employee` (IN `s` VARCHAR(250), IN `page` INT(5))  Begin
...
CREATE  PROCEDURE `employee_count` (IN `s` VARCHAR(50))  NO SQL
Begin
...
CREATE  PROCEDURE `form` (IN `page` INT(5))  begin
...
CREATE  PROCEDURE `form2` (IN `s` VARCHAR(50), IN `page` INT(5), IN `emp` VARCHAR(20))  NO SQL
begin
...
CREATE  PROCEDURE `form2_count` (IN `s` VARCHAR(50), IN `emp` INT(20))  NO SQL
begin
select f.form_id,f.emp_id,f.cus_id,form_date,stt_accept,amount,status,usr_acc,emp_name,company,packing_no,fd.code,p.img_path,form_time from form f left join employee e on f.emp_id=e.emp_id left join customer c on f.cus_id=c.cus_id left join formdetail fd on f.form_id=fd.form_id  left join products p on fd.code=p.code where f.form_id like s or f.emp_id like s or f.cus_id like s or form_date like s or stt_accept like s or amount like s or STATUS like s or usr_acc like s or emp_name like s or company like s or packing_no like s or fd.code like s or p.img_path like s or form_time like s and f.emp_id=emp order by form_date DESC;
end$$

CREATE  PROCEDURE `form_count` ()  NO SQL
begin
select f.form_id,f.emp_id,f.cus_id,form_date,stt_accept,amount,status,usr_acc,emp_name,company,packing_no,fd.code,p.img_path,form_time from form f left join employee e on f.emp_id=e.emp_id left join customer c on f.cus_id=c.cus_id left join formdetail fd on f.form_id=fd.form_id  left join products p on fd.code=p.code where stt_accept='ຍັງບໍ່ອະນຸມັດ' order by form_date DESC;
end$$

CREATE  PROCEDURE `form_detail` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `form_detail_count` (IN `s` VARCHAR(50))  NO SQL
begin
...
CREATE  PROCEDURE `insert_auther` (`autherid` VARCHAR(20), `authername` VARCHAR(50))  Begin
Insert into auther values(autherid,authername);
End$$

CREATE  PROCEDURE `insert_brand` (`brand_name` VARCHAR(50))  begin
insert into brand(brand_name) values(brand_name);
End$$

CREATE  PROCEDURE `insert_category` (`cate_name` VARCHAR(50))  begin
Insert into category (cate_name) values(cate_name);
end$$

CREATE  PROCEDURE `insert_check_stock` (`code` VARCHAR(30), `serial` VARCHAR(30), `qty` INT(11), `emp_id` VARCHAR(20), `check_date` DATE, `check_time` TIME, `pro_ad` INT(11), `remark` VARCHAR(100))  begin
...
CREATE  PROCEDURE `insert_customer` (`cus_id` VARCHAR(20), `company` VARCHAR(50), `tel` VARCHAR(30), `address` VARCHAR(250), `email` VARCHAR(100), `stt_id` VARCHAR(20))  begin
Insert into customer values(cus_id,company,tel,address,email,stt_id);
end$$

CREATE  PROCEDURE `insert_customer_status` (`sst_id` VARCHAR(20), `stt_name` VARCHAR(50))  begin
Insert into customer_status values(sst_id,stt_name);
end$$

CREATE  PROCEDURE `insert_distribute` (IN `code` VARCHAR(30), IN `serial` VARCHAR(30), IN `qty` INT(11), IN `emp_id` VARCHAR(20), IN `form_id` INT(11), IN `dis_date` DATE, IN `dis_time` TIME, IN `remark` VARCHAR(100))  begin
...
CREATE  PROCEDURE `insert_employee` (IN `empid` VARCHAR(20), IN `name` VARCHAR(50), IN `surname` VARCHAR(50), IN `gender` VARCHAR(10), IN `tel` VARCHAR(30), IN `address` VARCHAR(250), IN `email` VARCHAR(100), IN `pass` VARCHAR(100), IN `autherid` VARCHAR(20), IN `sttid` INT, IN `imgpath` VARCHAR(50))  Begin
Insert into employee values(empid,name,surname,gender,tel,address,email,md5(pass),autherid,sttid,imgpath);
End$$

CREATE  PROCEDURE `insert_form` (IN `form_id` INT(11), IN `emp_id` VARCHAR(20), IN `cus_id` VARCHAR(20), IN `amount` DECIMAL(11,2), IN `packing` INT(11), IN `form_date` DATE, IN `form_time` TIME, IN `stt_accept` VARCHAR(50), IN `status` VARCHAR(50), IN `usr_acc` VARCHAR(50))  begin
Insert into form values(form_id,emp_id,cus_id,amount,packing,form_date,form_time,stt_accept,status,usr_acc);
end$$

CREATE  PROCEDURE `insert_form_detail` (IN `code` VARCHAR(30), IN `qty` INT(11), IN `form_id` INT(11))  begin
insert into formdetail (code,qty,form_id) values(code,qty,form_id);
end$$

CREATE  PROCEDURE `insert_products` (`code` VARCHAR(30), `pro_name` VARCHAR(50), `gen` VARCHAR(50), `cate_id` INT(11), `unit_id` INT(11), `brand_id` INT(11), `qtyalert` INT(11), `img_path` VARCHAR(50))  begin
Insert into products values(code,pro_name,gen,cate_id,unit_id,brand_id,qtyalert,img_path);
end$$

CREATE  PROCEDURE `insert_product_addr` (`addr_name` VARCHAR(50))  begin
insert into product_addr(addr_name) values(addr_name);
end$$

CREATE  PROCEDURE `insert_product_putback_stock` (IN `code` VARCHAR(30), IN `serial` VARCHAR(30), IN `qty` INT(11), IN `emp_id` VARCHAR(20), IN `form_id` INT(11), IN `date_back` DATE, IN `time_back` TIME, IN `remark` VARCHAR(100))  begin
...
CREATE  PROCEDURE `insert_rate` (`rate_id` VARCHAR(20), `rate_buy` DECIMAL(11,2), `rate_sell` DECIMAL(11,2))  begin
insert into rate values(rate_id,rate_buy,rate_sell);
end$$

CREATE  PROCEDURE `insert_spare_part` (`emp_id` VARCHAR(20), `code` VARCHAR(30), `serial` VARCHAR(30), `spare_part` VARCHAR(50), `pro_id` VARCHAR(30), `pro_serial` VARCHAR(30), `spare_date` DATE, `spare_time` TIME, `remark` VARCHAR(100))  begin
...
CREATE  PROCEDURE `insert_stocks` (`code` VARCHAR(30), `serial` VARCHAR(30), `qty` INT(11), `price` DECIMAL(11,2), `dnv` VARCHAR(10), `imp_no` VARCHAR(10), `imp_date` DATE, `imp_time` TIME, `pro_no` VARCHAR(5), `rate_id` VARCHAR(20), `rate` DECIMAL(11,2), `emp_id` VARCHAR(20), `sup_id` VARCHAR(20), `remark` VARCHAR(100))  begin
...
CREATE  PROCEDURE `insert_supplier` (`sup_id` VARCHAR(20), `company` VARCHAR(50), `tel` VARCHAR(30), `fax` VARCHAR(30), `address` VARCHAR(250), `email` VARCHAR(100), `img_path` VARCHAR(50))  begin
Insert into supplier values(sup_id,company,tel,fax,address,email,img_path);
end$$

CREATE  PROCEDURE `insert_unit` (IN `unit_name` VARCHAR(50))  begin
Insert into unit (unit_name) values(unit_name);
end$$

CREATE  PROCEDURE `products` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `products_count` (IN `s` VARCHAR(50))  NO SQL
begin
...
CREATE  PROCEDURE `product_addr` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
select pro_ad,addr_name from product_addr where pro_ad like s or addr_name like s order by addr_name ASC limit page,15;
end$$

CREATE  PROCEDURE `product_putback_stock` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `rate` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `rate_count` (IN `s` VARCHAR(50))  NO SQL
begin
...
CREATE  PROCEDURE `spare_part` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `stocks` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `supplier` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
...
CREATE  PROCEDURE `supplier_count` (IN `s` VARCHAR(50))  NO SQL
begin
...
CREATE  PROCEDURE `unit` (IN `s` VARCHAR(250), IN `page` INT(5))  begin
select unit_id,unit_name from unit where unit_id like s or unit_name like s order by unit_name ASC limit page,15;
end$$

CREATE  PROCEDURE `unit_count` (IN `s` VARCHAR(50))  NO SQL
begin
select unit_id,unit_name from unit where unit_id like s or unit_name like s order by unit_name ASC;
end$$

CREATE  PROCEDURE `update_auther` (`autherid` VARCHAR(20), `authername` VARCHAR(50))  Begin
Update auther set auther_name=authername where auther_id=autherid;
End$$

CREATE  PROCEDURE `update_brand` (`brand_id_update` INT(11), `brand_name_update` VARCHAR(50))  begin
update brand set brand_name=brand_name_update where brand_id=brand_id_update;
end$$

CREATE  PROCEDURE `update_category` (`cate_id_update` INT(11), `cate_name_update` VARCHAR(50))  begin
Update category set cate_name=cate_name_update where cate_id=cate_id_update;
end$$

CREATE  PROCEDURE `update_check_stock` (`id_update` INT(11), `code_update` VARCHAR(30), `serial_update` VARCHAR(30), `qty_update` INT(11), `emp_id_update` VARCHAR(20), `check_date_update` DATE, `check_time_update` TIME, `pro_ad_update` INT(11), `remark_update` VARCHAR(100))  begin
...
CREATE  PROCEDURE `update_customer` (`cus_id_update` VARCHAR(20), `company_update` VARCHAR(50), `tel_update` VARCHAR(30), `address_update` VARCHAR(250), `email_update` VARCHAR(100), `stt_id_update` VARCHAR(20))  begin
...
CREATE  PROCEDURE `update_customer_status` (`stt_id_update` VARCHAR(20), `stt_name_update` VARCHAR(50))  begin
Update customer_status set stt_name=stt_name_update where stt_id=stt_id_update;
end$$

CREATE  PROCEDURE `update_distribute` (IN `id_update` INT(11), IN `code_update` VARCHAR(30), IN `serial_update` VARCHAR(30), IN `qty_update` INT(11), IN `emp_id_update` VARCHAR(20), IN `form_id_update` INT(11), IN `dis_date_update` DATE, IN `dis_time_update` TIME, IN `remark_update` VARCHAR(100))  begin
...
CREATE  PROCEDURE `update_employee` (IN `empid` VARCHAR(20), IN `name` VARCHAR(50), IN `surname` VARCHAR(50), IN `genders` VARCHAR(10), IN `tels` VARCHAR(30), IN `addresst` VARCHAR(250), IN `emails` VARCHAR(100), IN `passw` VARCHAR(100), IN `autherid` VARCHAR(20), IN `sttid` INT, IN `imgpath` VARCHAR(50))  Begin
...
CREATE  PROCEDURE `update_form` (IN `form_id_update` INT(11), IN `emp_id_update` VARCHAR(20), IN `cus_id_update` VARCHAR(20), IN `amount_update` DECIMAL(11,2), IN `form_date_update` DATE, IN `form_time_update` TIME, IN `stt_accept_update` VARCHAR(50), IN `status_update` VARCHAR(50), IN `usr_acc_update` VARCHAR(50))  begin
...
CREATE  PROCEDURE `update_form_detail` (`id_update` INT(11), `code_update` VARCHAR(30), `qty_update` INT(11), `form_id_update` INT(11))  begin
Update formdetail set code=code_update,qty=qty_update,form_id=form_id_update where id=id_update;
end$$

CREATE  PROCEDURE `update_products` (`code_update` VARCHAR(30), `pro_name_update` VARCHAR(50), `gen_update` VARCHAR(50), `cate_id_update` INT(11), `unit_id_update` INT(11), `brand_id_update` INT(11), `qtyalert_update` INT(11), `img_path_update` VARCHAR(50))  begin
...
CREATE  PROCEDURE `update_supplier` (`sup_id_update` VARCHAR(20), `company_update` VARCHAR(50), `tel_update` VARCHAR(30), `fax_update` VARCHAR(30), `address_update` VARCHAR(250), `email_update` VARCHAR(100), `img_path_update` VARCHAR(50))  begin
...
CREATE  PROCEDURE `update_unit` (`unit_id_update` INT(11), `unit_name_update` VARCHAR(50))  begin
