...
if(isset($_POST["da"]))
{
   $obj->select_stocks($_POST['da'],$_POST['db'],$page);
}
else
{
   $obj->select_stocks("0000-00-00",$Date,$page);
}
if(mysqli_num_rows($resultstocks) > 0)
{
 $output .= '
 ...
 ';
 while($row = mysqli_fetch_array($resultstocks))
 {
  $output .= '
   <tr  class="result">
   <td>'.$row["code"].'</td>
 ...
 mysqli_free_result($resultstocks);  
 ...
 if(isset($_POST["da"]))
 {
   $obj->select_stocks_count($_POST['da'],$_POST['db']);
 }
 else
 {
    $obj->select_stocks_count("0000-00-00",$Date);
 }
   $count = mysqli_num_rows($resultstocks_count);
   mysqli_free_result($resultstocks_count);  
   mysqli_next_result($conn);
   $a = ceil($count/15);
   if(isset($_POST['page'])){
      if($_POST['page'] > 1){
